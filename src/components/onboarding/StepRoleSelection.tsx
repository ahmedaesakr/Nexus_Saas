        ...
        >
                ...
                {roles.map((role) => (
                    <button
                        ...
                        className={`p-4 rounded-xl border transition-all text-left flex items-center gap-3 ${value === role.id
                                ? "bg-primary/10 border-primary text-white shadow-[0_0_15px_rgba(140,255,75,0.2)]"
            ...
            <div className="mt-8 flex justify-end">
                <button
                    onClick={onNext}
                    disabled={!value}
                    className="px-6 py-2 bg-primary text-black rounded-lg text-sm font-bold disabled:opacity-50 disabled:cursor-not-allowed hover:bg-primary-hover transition-colors btn-tactile"
                >
                    Next
                </button>
            </div>
