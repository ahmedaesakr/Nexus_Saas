                        ...
                        <div className="flex gap-4">
                            {['twitter', 'github', 'discord', 'linkedin'].map((social) => (
                                <a
                                    key={social}
                                    href={`#${social}`}
                                    className="w-10 h-10 rounded-full bg-white/5 hover:bg-primary hover:text-white text-gray-400 flex items-center justify-center transition-all"
                                    aria-label={`Follow us on ${social}`}
                                >
                                    <span className="material-symbols-outlined text-xl">
                                        {social === 'twitter' ? 'raven' : social}
                                        {/* Using raven as twitter icon placeholder, material symbols doesn't have brands */}
                                    </span>
                                </a>
                            ))}
                        </div>
